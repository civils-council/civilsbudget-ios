...
class Test_UIViewController_Swift: UIKitTestCase {
    ...
    func test_rejects_if_vc_has_no_promise_property() {
        ...
        p.error { error in
            if case UIViewControllerPMKError.NotPromisable = error {
                ex.fulfill()
            }
        }
    ...
    func test_rejects_if_promise_property_returns_nil() {
        ...
        p.error { error in
            if case UIViewControllerPMKError.NilPromisable = error {
                ex.fulfill()
            }
        }
        waitForExpectationsWithTimeout(1, handler: nil)
    ...
    func test_rejects_if_promise_property_casts_wrongly() {
        ...
        p.error { err in
            if case UIViewControllerPMKError.NotGenericallyPromisable = err {
                ex.fulfill()
            }
        }
        waitForExpectationsWithTimeout(1, handler: nil)
